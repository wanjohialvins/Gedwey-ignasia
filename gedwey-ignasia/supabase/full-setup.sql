-- ============================================================
-- GEDWEY IGNASIA — FULL SUPABASE SETUP (paste in SQL Editor)
-- Idempotent: safe to run on new or existing projects
-- ============================================================
...
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS mature_mode_age_verified BOOLEAN DEFAULT FALSE;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS mature_mode_enabled BOOLEAN DEFAULT FALSE;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS dev_mode BOOLEAN DEFAULT TRUE;
...
BEGIN
  ...
  SET couple_id = new_couple_id, partner_id = partner_profile.id, updated_at = now()
  WHERE id = auth.uid();

  UPDATE public.profiles
  SET couple_id = new_couple_id, partner_id = auth.uid(), updated_at = now()
...
-- Drop old policies (ignore errors on fresh DB)
DO $$ DECLARE r RECORD; BEGIN
  FOR r IN SELECT policyname, tablename FROM pg_policies WHERE schemaname = 'public'
    AND tablename IN ('profiles','couples','cards','discovery_sessions','sessions',
      'journal_entries','health_checkins','time_capsules','activity_logs','shared_items')
  LOOP
...
DROP POLICY IF EXISTS "Public can read Gedwey media" ON storage.objects;
...
-- ---------- OPTIONAL SEED CARDS ----------
INSERT INTO cards (text, category)
SELECT * FROM (VALUES
  ('What was your first impression of me?', 'discovery'),
  ('What is one thing you admire about your partner today?', 'relationship_health'),
  ('If we could travel anywhere tomorrow, where would you go?', 'fun'),
  ('What helps you feel most loved?', 'intimacy')
) AS v(text, category)
WHERE NOT EXISTS (SELECT 1 FROM cards LIMIT 1);

-- ---------- GAME CARDS (run seed-game-cards-data.sql after this for ~1650 prompts) ----------
...
-- ---------- COUPLE SONGS (shared music library) ----------
...
-- Done
SELECT 'Gedwey Ignasia full setup applied successfully.' AS status;
