...
export const GedweyLoader = ({
  ...
  // New rotation values for a futuristic spinner effect
  ...
  useEffect(() => {
    ...
    // Rotating outer and inner rings in opposite directions
  ...
  }, [bar, dotOne, dotThree, dotTwo, flame, heart, ring, shimmer, textGlow, outerRotation, innerRotation]);
  ...
  // Dynamic values depending on dark vs light system theme
  const bgClass = transparent ? '' : isDark ? 'bg-slate-950' : 'bg-[#fff8ec]';
  const mainTitleColor = isDark ? '#a5b4fc' : '#c0460a'; // Indigo 300 vs Orange
  ...
  // SVG color overrides
  const heartOuterFill = isDark ? '#1e1b4b' : '#8B1A00'; // Indigo 900 vs Dark Red
  const heartOuterStroke = isDark ? '#6366f1' : '#c0460a'; // Indigo 500 vs Orange
  const heartMiddleFill = isDark ? '#3730a3' : '#c0340a'; // Indigo 800 vs Red
  const heartInnerFill = isDark ? '#4f46e5' : '#d94a12'; // Indigo 600 vs Bright Orange
  const flameOuterColor = isDark ? '#f43f5e' : '#e07820'; // Rose 500 vs Orange
  const flameInnerColor = isDark ? '#f472b6' : '#f5a623'; // Pink 400 vs Amber
  const flameCoreColor = isDark ? '#fdf2f8' : '#fff5d0'; // Pink 50 vs Cream
